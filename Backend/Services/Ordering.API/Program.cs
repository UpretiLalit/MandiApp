...
try
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        Console.WriteLine("📊 Forcefully recreating Ordering tables...");
        
        // Drop all dependent tables first, then parent tables
        ...
        ");
        Console.WriteLine("✅ Dropped all old tables");
        
        // Verify tables are gone
        var tablesExist = await context.Database.ExecuteSqlRawAsync(@"
            SELECT COUNT(*) FROM information_schema.tables 
            WHERE table_name IN ('Carts', 'CartItems', 'Orders', 'OrderItems')
        ");
        Console.WriteLine($"📊 Remaining ordering tables: {tablesExist}");
        
        // Create fresh tables with correct types
        await context.Database.ExecuteSqlRawAsync(@"
            CREATE TABLE ""Carts"" (
        ...
        Console.WriteLine("✅ All Ordering tables created with INTEGER types");
        
        // Verify schema
        var cartsCount = await context.Carts.CountAsync();
        Console.WriteLine($"✅ Verification: Carts table accessible with {cartsCount} records");
    }
}
catch (Exception ex)
{
