...
foreach ($service in $services) {
    ...
    if (Test-Path $servicePath) {
        ...
        Write-Host "✗ $($service.Name) NOT FOUND at $servicePath" -ForegroundColor Red
    }
}
...
cd '$rootPath\Backend\Services\Identity.API'
Write-Host '🔐 Identity.API Starting...' -ForegroundColor Cyan
Write-Host 'Port: 5003' -ForegroundColor Yellow
Write-Host 'Swagger: http://localhost:5003/swagger' -ForegroundColor Green
Write-Host ''
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", @"
cd '$rootPath\Backend\Services\Marketplace.API'
Write-Host '🛒 Marketplace.API Starting...' -ForegroundColor Cyan
...
# Start Ordering.API
Start-Process powershell -ArgumentList "-NoExit", "-Command", @"
cd '$rootPath\Backend\Services\Ordering.API'
Write-Host '📦 Ordering.API Starting...' -ForegroundColor Cyan
Write-Host 'Port: 5002' -ForegroundColor Yellow
Write-Host 'Swagger: http://localhost:5002/swagger' -ForegroundColor Green
Write-Host 'SignalR Hubs:' -ForegroundColor Magenta
Write-Host '  - Price: http://localhost:5002/hubs/price' -ForegroundColor White
Write-Host '  - Tracking: http://localhost:5002/hubs/tracking' -ForegroundColor White
Write-Host ''
& 'C:\Program Files\dotnet\dotnet.exe' run
"@
...
# Start Logistics.Hub
Start-Process powershell -ArgumentList "-NoExit", "-Command", @"
cd '$rootPath\Backend\Services\Logistics.Hub'
Write-Host '🚚 Logistics.Hub Starting...' -ForegroundColor Cyan
Write-Host 'Port: 5004' -ForegroundColor Yellow
Write-Host 'Swagger: http://localhost:5004/swagger' -ForegroundColor Green
Write-Host ''
& 'C:\Program Files\dotnet\dotnet.exe' run
"@
...
Write-Host ""
Write-Host "🗄️ Database: Supabase (Connected ✓)" -ForegroundColor Cyan
Write-Host ""
